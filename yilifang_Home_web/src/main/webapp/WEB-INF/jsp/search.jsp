...
<script type="text/javascript">
    $(function () {
        ...
        $("span[class='next']").click(function () {
            var page = ${page }+1;
            var keyword = ${query};
            location.href="${pageContext.request.contextPath}/search?page="+page+"?keyword="+keyword;
        })
    })
</script>
<!-------面包线 linknav--------->
      ...
      <ul class="list-all">
         ...
         <li>
            <div class="l-wrap">
	            ...
	            <div class="price">
					<span><span class="p-now">￥<strong><fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${item.item_price / 100 }"/></strong></span><span class="p-nor"></span><span class="active" style="">直降</span></span>
				</div>
	            <div class="title-a">
	                <a class="trackref presaleSign_225865" href="http://localhost:8085/item/${item.id}.html" target="_blank">${item.item_title }</a>
	            </div>
	        	<div class="title-b" style=""><a class="trackref" href="http://localhost:8085/item/${item.id}.html" target="_blank">${item.item_sell_point }</a></div>
	            <div class="comment">
