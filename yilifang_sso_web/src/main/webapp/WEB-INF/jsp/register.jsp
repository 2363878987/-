...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/user/check/"+encodeURI(encodeURI(escape($("#regName").val())))+"/1?r=" + Math.random(),
	            	success : function(data) {// url中问号前边的数字用于确定数据类型,如1 代表用户名 2 代表手机号这样可以 用参数形式实现一个接口的共用
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
